    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->btnFilterPublisher, &QPushButton::clicked, this, &MainWindow::filterByPublisher);
    connect(ui->btnFilterISBN, &QPushButton::clicked, this, &MainWindow::filterByISBN);
}
...
void MainWindow::setupModel()
{
    ...
    model->setColumnCount(7); // Увеличиваем количество столбцов
    ...
    model->setHeaderData(4, Qt::Horizontal, "Издательство"); // Новый столбец
    model->setHeaderData(5, Qt::Horizontal, "ISBN");         // Новый столбец
    model->setHeaderData(6, Qt::Horizontal, "Страницы");     // Новый столбец
...
void MainWindow::addBook()
{
    ...
    int pages = QInputDialog::getInt(this, "Добавить книгу", "Количество страниц:", 1, 1, 10000);
    ...
    rowItems << new QStandardItem(author)
             ...
             << new QStandardItem(QString::number(pages));
...
void MainWindow::editBook()
{
    ...
    int pages = model->item(index.row(), 6)->text().toInt();

    EditBookDialog dialog(this);
    dialog.setBookData(author, title, year, genre, publisher, isbn, pages);
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        model->setItem(index.row(), 6, new QStandardItem(QString::number(dialog.getPages())));
    }
}
...
void MainWindow::filterByPublisher()
{
    QString publisher = QInputDialog::getText(this, "Фильтр по издательству", "Введите издательство:");
    if (!publisher.isEmpty()) {
        proxyModel->setFilterKeyColumn(4); // Фильтрация по столбцу "Издательство"
        proxyModel->setFilterFixedString(publisher);
    }
}

void MainWindow::filterByISBN()
{
    QString isbn = QInputDialog::getText(this, "Фильтр по ISBN", "Введите ISBN:");
    if (!isbn.isEmpty()) {
        proxyModel->setFilterKeyColumn(5); // Фильтрация по столбцу "ISBN"
        proxyModel->setFilterFixedString(isbn);
    }
}
